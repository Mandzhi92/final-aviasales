...
import { getSearchId, getTickets } from '../../store/asyncReducer';
import Ticket from '../Ticket';
import Button from '../Button';
import ErrorMessage from '../ErrorMessage/ErrorMessage';
import { sortList, applyFilters } from '../../support-function/support';
...
const CardList = ({ sortItem, searchId, filters, ticketsData, isLoading, getId, getTicketsData, error }) => {
  ...
  let data = ticketList.length ? (
    <>
      {ticketList}
      <Button onClick={() => setTicketsNumber(ticketsNumber + 5)} btnClass="show-more" />
    </>
  ) : (
